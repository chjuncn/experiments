...
representation_names = ['baseline',
                        ...
                        "randomStr+edge_id_v2"]  # Replace with your actual list
...
def different_appendingtokens_performance(graph_file_name, shot_num=0, position="before", error_log_file=""):
    ...
    for model in model_list:
        if model in ["mistralai/Mistral-7B-Instruct-v0.3", "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo","google/gemma-2-9b-it"]:
            continue
        for i, representation in enumerate(representation_list):
            if i not in [8, 9, 10, 14]:
                continue
            # if model != "mistralai/Mistral-7B-Instruct-v0.3":
            #     continue
            test_1hop(model, representation_names[i], representation_list[8], representation, shot_num=shot_num, is_abs=True, error_log_file=error_log_file)
...
file_name = "representations/graph_representation/res_log/gnp_n15_p20.txt"
different_appendingtokens_performance(file_name, error_log_file=file_name)
