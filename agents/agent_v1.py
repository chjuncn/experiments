...
class Agent:
    def __init__(self, proposers: list[str], aggregator: str):
        ...
        self.proposer_models = [
            TogetherHFAdaptor(model, apiKey=api_key) for model in proposers
        ]
...
def test():
    ...
    agent = Agent(modelFactory.getProposers(), modelFactory.getAggregators()[0])
    context = ""
    ...
    for i in range(3):
        answers = agent.answer(question1, context)
        joined = [
    ...
    print(context)
    final = agent.aggregate_answers(question1, context)
